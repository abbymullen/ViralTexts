...
class Issue():
	...
	def attribution(self,word):
		...
		Returns the phrase.
		"""
		#need to add negative lookup to weed out fake "Word. The PaperName" constructions.
		possibleAttr = [
...
def cleanup(attribution):
	...
	name_only = re.sub(r"T[bh][eso]","",name_only)
	return name_only
...
def paper_replace(line):
	...
	if re.search(r"Mo[nu]iteur",line):
		return re.sub(r"Mo[nu]i[telc]+ur","Moniteur",line)
	...
	if re.search(r"[DH]i[ts]p[ua]tch",line):
		return re.sub(r"[DH]i[ts]p[ua]tch","Dispatch",line)
	if re.search(r"Del[it]a",line):
		return re.sub(r"Del[it]a","Delta",line)
	if re.search(r"Chronicl[ce]",line):
		return re.sub(r"Chronicl[ce]","Chronicle",line)
	return line
...
if __name__=="__main__":
	...
	attr_list = open("NashU_attr_2015-03-29.txt", "a")
	place_names_dict = dict_reader("WDI_places.txt")

	for issue in issueyielder('tu_dolly.pbd.metatext'):
		paper = Issue(issue)
		date = paper.date()
		...
		for term in name_list:
			...
			if attr != "":
				...
				attr_list.write(date + '\t' + attr + '\t' + cleaner + '\t' + cleanest + '\n')
